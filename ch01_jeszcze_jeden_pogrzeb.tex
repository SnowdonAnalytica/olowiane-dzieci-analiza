...
%  "Znikającą klasę." — the chapter's final sentence.
%  Chapter 1 is COMPLETE.
%  See \section{Znikająca klasa / The Vanishing Class} at end of file.
%
...
%% ---------------------------------------------------------------
\section{Wieczne mieszkanie / Eternal Dwelling}
%% ---------------------------------------------------------------

\poltext{W naszej parafii pod koniec pogrzebu ksiądz zawsze
odmawiał modlitwę za tego spośród nas, który ,,następny
znajdzie wieczne mieszkanie w niebie i stanie przed
sprawiedliwym obliczem, ponieważ śmierć jest początkiem
lepszego życia, a nasza rozłąka ze zmarłymi jest
przejściowa``. Czy jakoś tak.}

\poltrans{In our parish, toward the end of the funeral, the
priest would always say a prayer for the one among us who
would ``next find eternal dwelling in heaven and stand before
the righteous countenance, for death is the beginning of a
better life, and our separation from the departed is
temporary.'' Or something like that.}

The passage shifts register entirely: away from the
cemetery walk and the medical inventory, back into the
ritual frame of the parish funeral. A priest's formula,
quoted --- approximately --- from memory. And then three
words that quietly detonate the quotation.

\textbf{W naszej parafii} --- \textit{in our parish}.
\textbf{Parafia} --- the Catholic parish, the administrative
and spiritual unit of neighbourhood life in Polish
Catholicism. \textbf{Nasza} --- \textit{our} --- the same
possessive that has carried the chapter's collective
weight throughout: \textit{nasza półka}, \textit{nasza
garstka żywych}, now \textit{nasza parafia}. The community
is always \textit{nasza}; it belongs to all of them and
to no one in particular.

\textbf{pod koniec pogrzebu} --- \textit{toward the end of
the funeral}. \textbf{Pod koniec} --- \textit{toward the
end, near the close} --- a temporal phrase: \textit{pod}
(under, near) + genitive \textit{koniec} (end). The ritual
has a fixed structure; this prayer has a fixed position
within it.

\textbf{ksiądz zawsze odmawiał modlitwę} --- \textit{the
priest would always say a prayer}. \textbf{Zawsze} ---
\textit{always} --- the imperfective \textbf{odmawiał}
confirms the habitual: not once but every time, the same
prayer, the same position, the same formula. \textbf{Odmawiać
modlitwę} --- \textit{to recite a prayer} --- \textit{odmawiać}
literally means to recite or decline (as in declining a
verb), the prayer rendered as a formula to be spoken
through rather than composed.

\textbf{za tego spośród nas} --- \textit{for the one among
us}. \textbf{Za} + genitive: the preposition of prayer and
intercession --- one prays \textit{za} someone, on their
behalf. \textbf{Spośród nas} --- \textit{from among us,
out of our number} --- genitive of \textit{my} (we) with
the preposition \textit{spośród} (from among). The priest
does not name the next to die; he prays for the one who
will be selected, the next name reached from the shelf.

\textbf{który ,,następny znajdzie wieczne mieszkanie w
niebie``} --- \textit{who will ``next find eternal dwelling
in heaven''}. \textbf{Następny} --- \textit{next, the
following one} --- a word the chapter has earned: after
\textit{znikali po kolei}, \textit{jeden po drugim}, the
sequence has a liturgical name now. \textbf{Wieczne
mieszkanie} --- \textit{eternal dwelling, eternal home}
--- \textit{mieszkanie} is the ordinary Polish word for
a flat or apartment, the unit of housing on an estate.
The priest's phrase lifts it into the theological register,
but the word keeps its domestic weight: heaven as a
\textit{mieszkanie}, as if one moves in there the way one
moves to a new flat in a new city. In this chapter, where
people have been leaving the estate for years, the image
is not merely conventional.

\textbf{stanie przed sprawiedliwym obliczem} ---
\textit{will stand before the righteous countenance}.
\textbf{Stanąć} (perfective: to stand, to come to stand)
+ \textbf{przed} + instrumental: to appear before.
\textbf{Oblicze} --- \textit{countenance, visage, face}
--- an elevated, archaic word for face, the kind used
in liturgical and biblical language. \textbf{Sprawiedliwym}
--- \textit{righteous, just} --- agreeing with
\textit{obliczem} in the instrumental.

\textbf{ponieważ śmierć jest początkiem lepszego życia}
--- \textit{for death is the beginning of a better life}.
\textbf{Ponieważ} --- the formal \textit{because} (compare
\textit{gdyż} from the book's opening paragraph).
\textbf{Początkiem} --- instrumental of \textit{początek}
(beginning): death \textit{is} a beginning, predicate
instrumental. The doctrinal claim stated plainly.

\textbf{a nasza rozłąka ze zmarłymi jest przejściowa} ---
\textit{and our separation from the departed is temporary}.
\textbf{Rozłąka} --- \textit{separation, parting} ---
a word with emotional weight: not mere distance but the
ache of being apart from someone. \textbf{Ze zmarłymi}
--- \textit{with the deceased}, instrumental plural of
\textit{zmarły} (the deceased, one who has died).
\textbf{Przejściowa} --- \textit{temporary, transient,
passing} --- from \textit{przejść} (to pass through);
the separation is a corridor, not a terminus.

\medskip

\textbf{Czy jakoś tak.} --- \textit{Or something like
that. Or words to that effect.} Three words that do
everything. \textbf{Czy} here is not the interrogative
\textit{czy} (whether, or in yes/no questions) but the
concessive \textit{czy}: \textit{or, or else, or
perhaps}. \textbf{Jakoś} --- \textit{somehow, in some
such way} --- the same softening, hedging particle as
\textit{coś}, used here to disclaim precision.
\textbf{Tak} --- \textit{so, thus, like that}.
Together: \textit{or thereabouts, or something along
those lines, I'm paraphrasing}.

The effect is surgical. The narrator has just rendered
the priest's consolation formula in full --- eternal
dwelling, righteous countenance, death as beginning,
separation as temporary --- and then appended \textit{czy
jakoś tak}, which withdraws the guarantee of accuracy
and, in doing so, withdraws something more. He is not
denying the formula; he is declining to be held
responsible for its exact wording. The theology may be
approximately right. Whether it applies to this
particular shelf, depleted in this particular way, at
this particular rate --- that is the question the three
words do not ask but make audible.

\medskip
\noindent\textit{Vocabulary note:} \textbf{Mieszkanie}
as both earthly flat and heavenly dwelling quietly
connects the two worlds the chapter moves between: the
\textit{bloki}, the estate, the ground-floor apartment
where Grzesio once set a fire --- and now this, the
\textit{wieczne mieszkanie w niebie}. People leave their
earthly \textit{mieszkanie} and move to the eternal one.
On this estate, they have been moving rather often.

%% ---------------------------------------------------------------
\section{Kto teraz? / Who's Next?}
%% ---------------------------------------------------------------

\poltext{Odmawiając dziś z różnym zaangażowaniem \textit{Ojcze
nasz}, patrzyliśmy uważnie na siebie. Kto teraz? On? Ona? Ja?}

\poltrans{Reciting the \textit{Our Father} today with varying
degrees of engagement, we watched each other carefully. Who's
next? Him? Her? Me?}

Two sentences. The first places the whole chapter's
living remainder inside the ritual, murmuring the Lord's
Prayer with imperfect attention while their eyes move
across the pews. The second is three pronouns and three
question marks, and it is the chapter's centre of gravity.

\textbf{Odmawiając} --- the imperfective active participle
of \textit{odmawiać}: \textit{reciting, in the act of
reciting}. The same verb as \textit{odmawiał modlitwę}
in the previous passage --- the priest's habitual
\textit{odmawiał} now becomes the congregation's present
\textit{odmawiając}, the prayer happening simultaneously
with the watching. While saying the words, they looked.
The participle holds both actions in parallel without
resolving which is primary.

\textbf{dziś} --- \textit{today}. Placing the scene in
the present tense of the narration, not the habitual
past. This funeral, this day, this \textit{Ojcze nasz}.

\textbf{z różnym zaangażowaniem} --- \textit{with varying
engagement, with differing degrees of involvement}.
\textbf{Zaangażowanie} --- from \textit{angażować} (to
engage, to involve), itself from French \textit{engager}
--- commitment, engagement, investment. \textbf{Różnym}
--- \textit{varying, different} --- instrumental of
\textit{różny}, agreeing with \textit{zaangażowaniem}.
The phrase is a masterpiece of polite understatement:
some people said the prayer; some moved their lips; some
were elsewhere entirely. The congregation's faith, or
its attention, or its relationship to the formula, is
distributed unevenly. \textit{Z różnym zaangażowaniem}
acknowledges this without judgement --- the administrative
phrase applied to an act of worship the way it might
be applied to meeting attendance.

\textbf{\textit{Ojcze nasz}} --- the Polish \textit{Our
Father}, italicised as a title. The prayer is identified
by its opening words, as in all Catholic traditions:
\textit{Ojcze nasz, któryś jest w niebie} --- \textit{Our
Father, who art in heaven}. Here it arrives unquoted,
unnamed except by title: the formula everyone knows,
the words that move through the mouth while the mind
does something else.

\textbf{patrzyliśmy uważnie na siebie} --- \textit{we
watched each other carefully, we looked attentively at
one another}. \textbf{Patrzyliśmy} --- first person
plural imperfective past: \textit{we were watching}, the
looking sustained throughout the prayer. \textbf{Uważnie}
--- \textit{carefully, attentively} --- from
\textit{uwaga} (attention, notice): the same root as
\textit{uwaga była uzasadniona} earlier. \textbf{Na
siebie} --- \textit{at each other, at one another} ---
the reflexive \textit{siebie} in the accusative, as in
\textit{do siebie podobni} from the Hanka passage:
mutual, reciprocal. Each of them watched, and each was
watched. The prayer was the occasion; the attention was
the content.

\medskip

\textbf{Kto teraz?} --- \textit{Who's next? Who now?}
No verb. \textbf{Kto} --- \textit{who}, the interrogative
pronoun in the nominative, waiting to be assigned to the
next slot. \textbf{Teraz} --- \textit{now} --- pressing
the question into the present moment, this funeral, this
pew, this prayer still in the mouth.

\textbf{On? Ona? Ja?} --- \textit{Him? Her? Me?} Three
pronouns, three question marks, each a complete sentence.
\textbf{On} --- masculine third person: one of the men,
one of the Groszeks or Walenckis. \textbf{Ona} ---
feminine: Hanka, Daisy, one of the women. \textbf{Ja}
--- first person: the narrator, the author, the man with
the three adverbs and no named disease. The sequence
moves from the other to the self in three steps, and
the final step is the one that gives the chapter its
nerve. He is not merely cataloguing a cohort's
disappearance; he is in the sequence himself, waiting
for his turn.

\medskip
The passage closes the chapter's first movement. The
\textit{obchód} began with Fiszer's inconvenient death
and worked its way through the dead, the lost, the
living-and-diminished, to the moment in the pew where
the survivors scan each other during the Lord's Prayer
with \textit{różne zaangażowanie} --- the prayer moving
through them on autopilot while their actual attention
is occupied with the arithmetic of \textit{nasza półka}.
The priest's formula promised that separation is
\textit{przejściowa}, temporary. The three pronouns
ask who will be first to test that promise.

\medskip
\noindent\textit{Grammar note:} The three-pronoun
sequence \textbf{On? Ona? Ja?} uses the nominative in
each case --- the subject case, the case of the actor,
the one who does. In the context of dying, the nominative
is unexpected: one might expect the genitive
(\textit{kogo?} --- of whom?) or the accusative
(\textit{kogo?} --- whom?), the cases of the affected
object. But Polish grammar puts the next to die in the
nominative, as subject, as agent of their own departure.
The grammar refuses to make them passive.

%% ---------------------------------------------------------------
\section{Dwa palce na krzyż / Two Fingers Crossed}
%% ---------------------------------------------------------------

\poltext{Groszek mamrotał modlitwę niemal bezgłośnie,
trzymając dwa palce na krzyż. Miałem ochotę uzupełnić jego
dyskretne sygnały, pokazując faka, ale odpuściłem.}

\poltrans{Groszek was mumbling the prayer almost soundlessly,
holding two fingers crossed. I felt like supplementing his
discreet signals by giving him the finger, but I let it go.}

The trapdoor opens. After the existential vertigo of
\textit{Kto teraz? On? Ona? Ja?} the camera cuts to
Groszek mumbling the \textit{Ojcze nasz} with his
fingers crossed, and the narrator's suppressed impulse
to respond in kind. The chapter breathes again.

\textbf{mamrotał modlitwę niemal bezgłośnie} ---
\textit{was mumbling the prayer almost soundlessly}.
\textbf{Mamrotać} --- \textit{to mumble, to mutter} ---
an expressive onomatopoeic verb, the sound already in
the word. \textbf{Niemal} --- \textit{almost, nearly}
--- more literary than the everyday \textit{prawie},
a slight elevation in register that plays against the
mundanity of the action. \textbf{Bezgłośnie} ---
\textit{soundlessly, without voice} --- from
\textit{bez} (without) + \textit{głos} (voice). The
prayer is technically present; acoustically, barely.
This is Groszek's \textit{różne zaangażowanie}
specified: he is on the low end of the distribution.

\textbf{trzymając dwa palce na krzyż} --- \textit{holding
two fingers crossed}. \textbf{Trzymać} ---
\textit{to hold, to keep}. \textbf{Dwa palce} ---
\textit{two fingers}, accusative. \textbf{Na krzyż}
--- literally \textit{crosswise, in a cross} ---
the standard Polish description of crossed fingers:
\textit{trzymać kciuki} is to hold thumbs (the Polish
equivalent of fingers crossed for luck), but
\textit{palce na krzyż} is specifically the
gesture of crossing fingers behind one's back to
nullify a promise or, in folk belief, to ward off
the evil eye. Groszek is saying the prayer and
simultaneously withdrawing his assent from it. The
gesture is a five-hundred-year-old get-out clause,
deployed inside a Catholic funeral in full view
of the narrator.

\textbf{Miałem ochotę} --- \textit{I felt like, I had
an urge to}. \textbf{Mieć ochotę} + infinitive is
the standard construction for \textit{to feel like
doing something, to be tempted to}: literally
\textit{to have a desire/appetite}. \textbf{Ochotę}
--- accusative of \textit{ochota} (desire, inclination,
appetite) --- the word has an old-fashioned, slightly
jovial flavour.

\textbf{uzupełnić jego dyskretne sygnały} ---
\textit{to supplement his discreet signals}.
\textbf{Uzupełnić} --- perfective of \textit{uzupełniać}:
to complete, to supplement, to add to. \textbf{Dyskretne
sygnały} --- \textit{discreet signals} --- the narrator
treats Groszek's crossed fingers as a semiotic act,
a communication that deserves a reply in kind.
The formality of \textit{dyskretne sygnały} for
two old friends exchanging gestures during a
funeral is characteristically dry.

\textbf{pokazując faka} --- \textit{by showing the
fig, by giving him the finger}. \textbf{Fak} (also
\textit{figa}) --- the \textit{fig sign}: the fist
with the thumb protruding between index and middle
fingers, an ancient Mediterranean obscene gesture
imported into Slavic cultures, where it serves as
both an insult and a magical apotropaic sign.
In Polish folklore, \textit{pokazać figę} or
\textit{pokazać faka} is to give someone nothing
while pretending to give them something --- a
gesture of refusal, contempt, or mischief depending
on context. Here, replying to crossed fingers
with a \textit{fak} would be: your superstition
matched with mine, your \textit{don't hold me to
this} met with \textit{same to you}.

\textbf{ale odpuściłem} --- \textit{but I let it go,
I gave it a pass, I didn't bother}.
\textbf{Odpuścić} --- perfective of \textit{odpuszczać}:
to release, to let off, to give up on. In colloquial
Polish, \textit{odpuścić} means to decide not to
pursue something --- \textit{odpuść sobie} is a
common expression meaning \textit{let it go, don't
bother, give it a rest}. The narrator imagined the
gesture, weighed it, and declined. The restraint
is its own comedy: the situation warranted a
\textit{fak}; he chose decorum.

\medskip
The passage is a palate-cleanser and a portrait
simultaneously. After the chapter's most exposed
moment --- the narrator placing himself in the queue
with \textit{Ja?} --- Groszek provides an escape
hatch. He is, as ever, the most fully alive person
in the scene: mumbling a prayer he is simultaneously
cancelling, hedging his theological bets with a
medieval gesture while the priest delivers the
formula above him. He is Groszek. The narrator
has known him since the kiosk comics and the bushes
and fifth grade. Of course he almost showed him
a \textit{fak} during the funeral. Of course he
didn't.

\medskip
\noindent\textit{Cultural note:} The pairing of
\textbf{palce na krzyż} (fingers crossed, to nullify)
and \textbf{fak} (fig sign, to refuse or mock) places
two strands of pre-Christian folk magic inside a
Catholic liturgy, which is precisely where Polish
popular religion has always kept them. The Church
and the gesture have coexisted for centuries; Groszek
is not a rebel but a practitioner of something much
older and more pragmatic than doctrine.

%% ---------------------------------------------------------------
\section{Pogawędki o chorobach / Chats About Ailments}
%% ---------------------------------------------------------------

\poltext{Idąc wolno cmentarną alejką, utkwiliśmy w ulubionych
przez dojrzałych ludzi pogawędkach o chorobach. Wszak w pewnym
wieku każdy jest wybitnym ekspertem od siedmiu boleści. Daisy
jednak wzięło się na dalsze wspominki:}

\poltrans{Walking slowly along the cemetery path, we got
stuck in the conversations about illnesses that mature people
love. After all, at a certain age everyone is a distinguished
expert in all manner of aches and pains. But Daisy took it
upon herself to move on to further reminiscences:}

The group has left the church and resumed the walk.
The tone shifts into a wry sociological aside before
Daisy interrupts it with a colon.

\textbf{Idąc wolno cmentarną alejką} --- \textit{walking
slowly along the cemetery path}. \textbf{Idąc} ---
imperfective active participle of \textit{iść} (to go,
to walk): the walking is continuous, the backdrop to
everything that follows. \textbf{Wolno} ---
\textit{slowly} --- the pace of a funeral procession
after the service, unhurried. \textbf{Cmentarną alejką}
--- instrumental of \textit{cmentarna aleja}: the
cemetery path or lane. \textbf{Aleja} --- from French
\textit{allée} --- suggests something more formal than
a mere path: a lined, purposeful walkway. The
instrumental here marks manner or path of motion:
one walks \textit{by means of} the alley, along it.

\textbf{utkwiliśmy w\ldots{} pogawędkach} ---
\textit{we got stuck in\ldots{} conversations, we
became lodged in chats}. \textbf{Utkwić} --- perfective
of \textit{tkwić} (to be stuck, to be embedded, to be
lodged): to get stuck, to become fixed. The verb is
used for things that embed themselves: a splinter
\textit{tkwi} in the finger; a thought \textit{tkwi}
in the mind. Applied to a conversation, it implies the
group was not merely chatting but had settled into the
talk the way one settles into a groove, unable or
unwilling to get out. \textbf{W pogawędkach} ---
locative plural: \textit{lodged in chats}.

\textbf{ulubionych przez dojrzałych ludzi pogawędkach
o chorobach} --- \textit{conversations about illnesses
beloved by mature people}. The participial phrase
\textbf{ulubionych przez dojrzałych ludzi} ---
\textit{beloved/favoured by mature people} --- wraps
the noun with gentle condescension. \textbf{Ulubiony}
--- \textit{favourite, beloved} --- from \textit{lubić}
(to like). \textbf{Dojrzały} --- \textit{mature, ripe,
of a certain age} --- the polite word for
\textit{middle-aged to elderly}: literally
\textit{ripened}, as of fruit. The narrator includes
himself in this category without complaint but with
visible amusement.

\textbf{Wszak} --- \textit{after all, indeed, you
know}. An older, slightly formal connective, rarer
in contemporary speech than \textit{przecież} or
\textit{w końcu}: it introduces a self-evident truth
being offered as explanation. \textbf{Wszak} has a
mild archaic warmth; it sounds like a grandfather
making a reasonable point.

\textbf{w pewnym wieku} --- \textit{at a certain age}.
\textbf{Pewnym} --- \textit{certain, a particular} ---
the age left unnamed, recognised by all. The locative
\textbf{w wieku} (in age, at an age) is the standard
construction.

\textbf{każdy jest wybitnym ekspertem od siedmiu
boleści} --- \textit{everyone is a distinguished expert
in all manner of aches and pains}. \textbf{Wybitny}
--- \textit{distinguished, outstanding, eminent} ---
a word for great scientists and professors, here
applied to the self-appointed authority of the
chronically unwell. \textbf{Ekspert} --- the
international word, unambiguous. \textbf{Od siedmiu
boleści} --- literally \textit{from/of seven pains}:
an idiomatic intensifier, a fixed phrase meaning
\textit{of every conceivable kind, of all sorts}.
\textbf{Siedem} (seven) appears in many Polish
intensifying idioms; \textit{siedem boleści}
(seven pains/sorrows) carries a faint echo of the
Seven Sorrows of Mary in Catholic tradition, but
functions here purely as hyperbolic enumeration.
Everyone past a certain age has opinions about their
condition; everyone is an authority on their own
suffering.

\textbf{Daisy jednak wzięło się na dalsze wspominki}
--- \textit{but Daisy took it upon herself to move on
to further reminiscences}. \textbf{Jednak} ---
\textit{however, but, yet} --- the pivot: the illness
conversation is the expected mode; Daisy breaks from
it. \textbf{Wziąć się na coś} --- a colloquial
construction: \textit{to set oneself to something,
to take it upon oneself}. \textbf{Dalsze wspominki}
--- \textit{further reminiscences}: \textbf{wspominki}
is the diminutive plural of \textit{wspomnienia}
(memories, reminiscences) --- the \textit{-ki} suffix
makes them smaller, cosier, slightly nostalgic in
register, the kind of memories shared among friends
rather than recorded in testimony.

\medskip
\noindent\textit{Grammar note:} \textbf{Daisy jednak
wzięło się} uses the neuter form \textbf{wzięło}
rather than the feminine \textbf{wzięła}. This is
notable: Daisy is a woman, and a feminine past tense
would be expected. The neuter \textit{wzięło} is
the narrator's solution to an English name that
Polish grammar cannot automatically assign a gender
to --- \textit{Daisy} ends in nothing Polish
morphology recognises as a feminine marker, so the
neuter becomes the unmarked default. It is a small
collision between an English name and a grammatical
system that requires every person to be
masculine or feminine in the past tense.

\medskip
The colon that closes the passage announces Daisy's
voice directly. Having introduced her earlier as
the member of the cohort with \textit{choroba układu
odpornościowego zwana też autoagresją}, the narrator
now gives her the floor. The shift from the group's
comfortable \textit{pogawędki o chorobach} to Daisy's
specific \textit{wspominki} signals that what follows
will be more particular than the general ailment-
expertise of the \textit{dojrzali ludzie}.

%% ---------------------------------------------------------------
\section{Jak znikały dzieci / How the Children Disappeared}
%% ---------------------------------------------------------------

\poltext{--- A pamiętacie, jak w podstawówce zniknęła połowa
klasy? Jak musieli z dwóch klas, piątej A i piątej B, zrobić
jedną? Jak znikały dzieci?}

\poltrans{``And do you remember how in primary school half
the class disappeared? How they had to make one class out of
two, fifth A and fifth B? How the children disappeared?''}

Three questions, each beginning with \textbf{jak}, each
enlarging the frame. Daisy does not announce her subject;
she approaches it through shared memory, testing whether
the others hold the same images. The third question is
the one that matters, and she places it last with the
precision of someone who has been thinking about this
for a long time.

\textbf{A pamiętacie} --- \textit{and do you remember}.
\textbf{A} again as opener, the particle of gentle
transition and address. \textbf{Pamiętacie} --- second
person plural: she is speaking to the group, to Groszek
and the narrator together, appealing to shared memory.
The verb of remembering has governed this whole chapter;
\textit{pamiętasz ich jeszcze?} asked Groszek of the
narrator; now Daisy asks it of everyone.

\textbf{jak w podstawówce zniknęła połowa klasy} ---
\textit{how in primary school half the class disappeared}.
The first \textbf{jak} is not \textit{how} in the
manner-sense but the \textit{jak} of reported memory:
\textit{do you remember the time when, do you remember
how it was that}. \textbf{Podstawówka} --- the same
affectionate colloquial word as the book's opening
sentence (\textit{kolega z podstawówki}) --- the
intimacy of the diminutive form keeps the memory
close. \textbf{Zniknęła} --- perfective past of
\textit{znikać}: completed, definitive disappearance.
Note the contrast with the earlier \textbf{znikali}
(imperfective, they kept disappearing) and
\textbf{znikały} at the end of this passage: the
perfective \textit{zniknęła} here applies to
\textit{połowa klasy} --- the half-class vanished as
a unit, a single event. \textbf{Połowa klasy} ---
\textit{half the class}: not one or two students
but half.

\textbf{Jak musieli z dwóch klas, piątej A i piątej B,
zrobić jedną?} --- \textit{How they had to make one
class out of two, fifth A and fifth B?} The unnamed
\textbf{musieli} --- \textit{they had to} --- is the
school administration, the teachers, whoever makes
such decisions. The classes are named with the
specificity of a school register: \textbf{piąta A}
and \textbf{piąta B} --- fifth grade A and fifth grade
B. In Polish schools, parallel classes are labelled
alphabetically; to name both is to insist on the
precision of the memory, to say: this is not a general
impression but a specific institutional fact. Two
classes became one because there were no longer enough
children to fill two. The arithmetic of the
disappearance is administrative as well as human.

\textbf{Jak znikały dzieci?} --- \textit{How the
children disappeared?} The third and final question,
and the pivot on which everything turns.
\textbf{Znikały} --- imperfective past of
\textit{znikać}: not a single event but an ongoing
process, the same aspect as \textit{znikali po kolei}
earlier. The children kept disappearing, one after
another, over time. And the subject: \textbf{dzieci}
--- \textit{children}. The book is called
\textit{Ołowiane dzieci} --- \textit{Lead Children}.
The subtitle is \textit{Zapomniana epidemia} ---
\textit{The Forgotten Epidemic}. Daisy, walking along
a cemetery path in middle age, has just spoken the
book's subject into the text for the first time.
She does not explain. She asks.

\medskip
\noindent\textit{Grammar note:} The three questions
share the structure \textbf{pamiętacie, jak\ldots{}}
--- \textit{do you remember how\ldots{}} --- with
\textbf{jak} introducing each subordinate clause.
This \textbf{jak} is not the interrogative of manner
(\textit{how does one do it?}) but the \textbf{jak}
of memory and narrative, equivalent to \textit{the
time when} or \textit{the way that}: \textit{do you
remember the way half the class disappeared?} It is
the grammar of shared witness: I remember it, do you?

\medskip
The three-question structure is a rhetorical
crescendo. The first question establishes the fact
(\textit{half the class gone}); the second gives it
institutional confirmation (\textit{two classes
merged into one}); the third strips away the
administrative language and names what was actually
happening: \textit{jak znikały dzieci}. Children
were disappearing. Daisy, the one with the immune
system turned against itself, walking at a funeral
for a classmate, asking the group beside her whether
they remember. The question is also an accusation,
also a grief, also a demand to finally pay attention
to something that was happening in plain sight.

The \textit{obchód} has found its subject. The
\textit{pogawędki o chorobach} were the warm-up;
this is what Daisy came to say.

%% ---------------------------------------------------------------
\section{Znikająca klasa / The Vanishing Class}
%% ---------------------------------------------------------------

\poltext{To prawda. Już kiedyś znikały. Z klasy.
Z dziennika. Z listy obecności. Ze szkoły. Współczesne
groby przypomniały nam znikające dzieci sprzed wielu lat.
Znikającą klasę.}

\poltrans{It's true. They were already disappearing back
then. From the class. From the register. From the
attendance list. From the school. The contemporary graves
reminded us of the disappearing children of many years
ago. The vanishing class.}

The chapter ends. The narrator confirms Daisy's
memory in two words, then enacts the disappearance
in the grammar itself --- four prepositional phrases,
each one a further erasure --- before the long
sentence that joins past to present, and the final
two-word fragment that names what was lost.

\textbf{To prawda.} --- \textit{It's true.}
\textbf{To} --- \textit{it, that} --- the flat
demonstrative as affirmation. No elaboration, no
hedging. The narrator has heard Daisy's three
questions and has two words in answer: yes, that
happened, I remember it too. The brevity is the
weight.

\textbf{Już kiedyś znikały.} --- \textit{They were
already disappearing back then.} \textbf{Już} ---
\textit{already} --- pressing the disappearance
back in time: it was happening then, not just now.
\textbf{Kiedyś} --- \textit{back then, once, at
some point in the past} --- the same indefinite
past-marker as \textit{kiedyś podpalił drzwi Aście},
but now carrying the weight of childhood. \textbf{Znikały}
--- imperfective past, neuter plural (agreeing with
\textit{dzieci}, which takes neuter plural agreement):
they kept disappearing, the process ongoing and
unfinished even in memory.

\medskip

Then the four fragments:

\textbf{Z klasy.} --- \textit{From the class.}\\
\textbf{Z dziennika.} --- \textit{From the register.}\\
\textbf{Z listy obecności.} --- \textit{From the
attendance list.}\\
\textbf{Ze szkoły.} --- \textit{From the school.}

Each is a prepositional phrase with no verb, no
subject --- pure direction of removal. \textbf{Z} /
\textbf{Ze} + genitive: the preposition of departure,
of extraction, of \textit{from}. A child disappears
\textit{z klasy} (from the class --- the social unit,
the group of children), then \textit{z dziennika}
(from the class register --- the official paper
record, the \textit{dziennik lekcyjny}), then
\textit{z listy obecności} (from the attendance
list --- the daily roll-call, the document of who
is present), then \textit{ze szkoły} (from the school
entirely). The sequence is an institutional
bureaucracy of erasure, moving from the social
(\textit{klasa}) to the administrative (\textit{dziennik})
to the procedural (\textit{lista obecności}) to the
physical (\textit{szkoła}). Each step removes the child
from another layer of the record. By the time
\textit{ze szkoły} arrives, there is nothing left
to be removed from.

\medskip
\noindent\textit{Grammar note:} \textbf{Ze} is the
phonetic variant of \textbf{z} used before consonant
clusters that would be difficult to pronounce
otherwise: \textit{ze szkoły}, \textit{ze mną},
\textit{ze sobą}. The preposition is the same;
the vowel is added for euphony. Polish is consistent
about this: \textit{z} + voiced sibilant or certain
clusters becomes \textit{ze}.

\medskip

\textbf{Współczesne groby przypomniały nam znikające
dzieci sprzed wielu lat.} --- \textit{The contemporary
graves reminded us of the disappearing children of
many years ago.} \textbf{Współczesne} ---
\textit{contemporary, present-day} --- from
\textit{współ-} (co-, together with) + \textit{czesny}
(of the time): \textit{of the same time as us, of
our era}. The graves they have been walking among
all chapter --- Fiszer's, Gutek's, the unnamed
others --- are \textit{współczesne}: their generation's
graves, the ones accumulating now. \textbf{Przypomniały
nam} --- \textit{reminded us}: the graves performed
the act of memory, triggered the recollection.
\textbf{Znikające dzieci} --- \textit{the disappearing
children}: present active participle of \textit{znikać}
used as an adjective, \textit{znikające} --- children
in the act of disappearing, the disappearance
characterising them. Not \textit{zniknięte dzieci}
(the disappeared children, passive, completed) but
\textit{znikające}: still vanishing, the participle
holding them in process. \textbf{Sprzed wielu lat}
--- \textit{from many years ago}: \textit{sprzed}
(from before) + genitive: \textit{from before
many years}.

\textbf{Znikającą klasę.} --- \textit{The vanishing
class.} Two words, full stop. The final sentence of
the chapter is a fragment: an accusative noun phrase,
the object of \textit{przypomniały} extended and
isolated. The graves reminded us of the disappearing
children --- reminded us, more precisely, of
\textit{znikającą klasę}: the class that was
vanishing. \textbf{Znikającą} --- accusative feminine
singular of \textit{znikający} (the same present
active participle): agreeing with \textbf{klasę},
the class. The class is still in the act of vanishing;
the participle refuses completion.

\medskip
This is the chapter's title inverted and revealed.
The chapter is called \textit{Jeszcze jeden pogrzeb}
--- \textit{Yet Another Funeral} --- but it ends with
\textit{znikającą klasę}, the vanishing class, which
is the book's subject named at last. The \textit{obchód}
was never only a walk among graves: it was a method
of reconstruction, a way of counting, a slow arithmetic
that arrives here, at the end, at its conclusion.
The contemporary dead reminded them of the children.
The children were already vanishing then. From the
class. From the register. From the school. One
after another, in sequence, \textit{po kolei}.

\medskip
\noindent\textit{On the participle:} The chapter's
final word is \textbf{klasę}, but its final modifier
is \textbf{znikającą} --- the present active
participle, the form that describes an action still
in progress at the time being referred to. The
vanishing class is still vanishing: the participle
will not let it stop. Polish grammar here does
something that a simple past adjective could not:
it holds the children in the moment of disappearance,
neither present nor safely past, the process
unresolved. \textit{Znikającą klasę.} The chapter
closes on an action that has not yet finished.
